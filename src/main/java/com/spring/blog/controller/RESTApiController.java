...
//@Controller// 컨트롤러로 지정
//@ResponseBody // REST형식 전환, 메서드 위에 붙으면 해당 메서드를 REST형식으로
...
public class RESTApiController {
    ...
    public PersonDTO person(){
        PersonDTO p = new PersonDTO().builder().id(1L).name("좋코더").age(20).build();
        return p;
    }

    // 상태코드까지 함께 리턴할 수 있는 ResposeEntity<> 를 리턴자료형으로 지정.
    @GetMapping("/person-list")
    public ResponseEntity<?> personList(){
        PersonDTO p = PersonDTO.builder().id(1L).name("꺄깎").age(10).build();
        PersonDTO p2 = PersonDTO.builder().id(2L).name("짝짝").age(20).build();
        PersonDTO p3 = PersonDTO.builder().id(3L).name("호호").age(30).build();
